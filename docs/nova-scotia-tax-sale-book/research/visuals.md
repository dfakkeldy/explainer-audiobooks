...
## Interactive-map walkthrough batch — 2026-07-19

Figures 41 through 51 are version-stamped 2560-by-1440 review candidates for
the proposed Chapter 5. They show the layer controls, Province-data licence,
municipal/event filtering, exact PID search, each available provincial layer,
and a combined parcel-research state. Their source commit, hashes and refresh
boundary are recorded in `figures/map-chapter-screenshot-receipt.json`; the
contact sheet is `figures/map-chapter-screenshots-contact-sheet.png`.
...
| `figure-41-map-layer-overview.png` | 5 | Establish the map controls and available layers before adding provincial data. | Screenshot of NS Marks The Spot source commit `1ee76d15b`; full provenance in the screenshot receipt. | Start with the source, event and question before choosing a visually interesting layer. | Province-wide map with both event catalogues and all map-layer toggles visible in their initial state. |
...
| `figure-43-tax-sale-notice-filter.png` | 5 | Select the Inverness event and redemption category while keeping the direct official source visible. | Screenshot of the dated municipal catalogue snapshot; refresh required. | Event filters organize a municipal notice; they do not determine live legal status. | Control panel with municipality, event date, snapshot note, direct-source link and redeemable-category filter. |
| `figure-44-pid-search-parcel-inspector.png` | 5 | Search one exact PID and separate municipal facts, mapped area and intersection results. | Screenshot using PID `50203256`; dated interface demonstration only. | Read the inspector's bounded words before interpreting the parcel shape. | Selected Mabou parcel with notice fields, approximate acreage and empty mapped-intersection states. |
| `figure-45-aerial-and-property-boundaries.png` | 5 | Compare aerial context with graphical parcel boundaries. | Screenshot using NS Aerial and NSPRD services under the in-app attribution. | Imagery plus a boundary service is still neither a survey nor current site inspection. | Detailed Mabou aerial with selected property-boundary linework and full layer controls. |
| `figure-46-roads-trails-culverts.png` | 5 | Reveal mapped transportation classes and their legend. | Screenshot of the Province roads/trails/culverts overlay. | A mapped line or polygon intersection is not legal access, maintenance or permission to enter. | Roads, trail classes and culvert symbols over selected property boundaries. |
| `figure-47-water-features.png` | 5 | Reveal mapped rivers, lakes, wetlands and related hydrography. | Screenshot of the Province water-features overlay. | Water linework is a screening clue, not a wetland delineation, water-quality finding or setback decision. | Water-feature symbology over selected property boundaries near Mabou. |
| `figure-48-crown-lands.png` | 5 | Show Crown-land context without changing the selected parcel's identity. | Screenshot of the Province Crown Lands overlay. | Nearby Crown land does not establish ownership of or access to the tax-sale parcel. | Crown-land overlay and graphical property boundaries in the Mabou research view. |
| `figure-49-flood-risk-areas.png` | 5 | Demonstrate watershed-scale flood-risk screening and service coverage. | Screenshot of the Province flood-risk overlay. | A screening service is not a parcel-specific flood certificate, insurance decision or future guarantee. | Flood-risk watershed linework in the parcel research view with source attribution intact. |
| `figure-50-waterfalls-overview.png` | 5 | Demonstrate a regional context layer and the discipline of choosing only question-relevant data. | Screenshot of the Province waterfalls overlay. | Not every available layer belongs in every parcel file. | Nova Scotia-wide waterfall distribution with the control panel and attribution visible. |
| `figure-51-combined-parcel-research.png` | 5 | Close the walkthrough by comparing visible context with exact mapped-intersection results. | Screenshot using PID `50308311`; dated interface demonstration only. | “Mapped water intersects; no mapped road intersects” is a research note, not an access, flood or site-condition conclusion. | Southside River Denys parcel with property, water and transport layers active and the exact-result inspector open. |
