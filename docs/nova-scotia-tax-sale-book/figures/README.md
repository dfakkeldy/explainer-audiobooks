...
The deterministic batch covers figures 03 through 08 plus figures 39 and 40:
the legal/process and record-research sequence for Chapters 1–3, the proposed
Chapter 10 payment-performance clock and Chapter 12 surplus-proceeds route. They are
...
The proposed Chapter 5 adds eleven browser-captured review candidates,
`figure-41` through `figure-51`, plus
`map-chapter-screenshots-contact-sheet.png`. Their source commit, capture
boundary and SHA-256 values are recorded in
`map-chapter-screenshot-receipt.json`. They are not generated by the diagram
renderer and must be recaptured after the map interface freezes.
