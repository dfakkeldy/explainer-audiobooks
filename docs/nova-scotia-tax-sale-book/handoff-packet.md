...
- Prior knowledge: ordinary real-estate-listing familiarity, but no assumed tax
  sale, registry, title, redemption or municipal-law expertise.
- Listening mode: road-book.
- Primary listening context: driving and delivering mail when safe.
- Attention constraints: eyes unavailable, interruptions, one-pass listening,
  and high-stakes distinctions requiring concrete resets.
- Revision mode: new book.
...
- Canonical drafting: Chapters 1–11 complete at 26,848 Markdown words; Chapter 12
...
The listener can explain the legal sequence, read municipal terminology, build
a bounded evidence file, recognize the auction and certificate-holder failure
paths, and identify the questions that belong with a lawyer, surveyor, planner,
insurer, accountant or other qualified professional.

## Opening orientation

- Context: Inverness County already publishes a substantial parcel packet.
- Promise: show what it establishes, what remains unknown and what additional
  source-traceable research can responsibly add.
- Route: travel backward from the packet to lien and notice, then forward
  through auction or tender, payment, certificate, possible redemption, deed,
  contestability and post-deed responsibility.

## Boundaries

Educational only; no legal, tax, insurance, investment, appraisal, survey,
access, environmental, planning, tenancy or construction advice. No property
recommendation, personalized bid, trespass instruction, owner narrative or
promise that a certificate interest can be insured. Halifax must be taught
under the HRM Charter parallel rather than silently folded into the MGA.
...
## Throughlines and concept paths

Use the four throughlines in `research/learning-outline.json` and every complete
definition–mechanism–case–boundary–misconception–retrieval path in
`research/coverage-ledger.json`. `research/coverage-ledger.md` is the readable
index. The payment-performance path must never inherit the staged-responsibility
outcome by copy and paste.
...
`research/evidence-notes.json` is the traceable claim authority and is
hash-bound to `research/evidence-notes.md`. Primary sources control legal
claims. Municipal event pages, fees, insurer examples and current auction lists
must be refreshed during drafting and again before publication.

## Section drafting and revision contract

Draft one approved section at a time with the full outline, exact claim IDs,
...
The pending plan is `research/pronunciation-plan.json`. Required probes include
Mabou, Whycocomagh, Judique, AAN, PID, NSPRD and MGA, including acronym versus
expanded spoken forms. Candidate readings are not acceptance evidence.
...
- Avoid false drama, treasure-hunt language, clipped pseudo-profundity, repeated
  “not X but Y” constructions and generic honesty announcements.
- The recurring researcher organizes evidence and questions; the fictional
  bidder owns every bid or walk-away decision.
...
Only after all human gates: complete Markdown and EPUB with accepted figures;
cover; Echo narration and alignment sidecar; pronunciation receipt; learning,
prose, visual and public-safety receipts; M4B; README and manifest; verified
public delivery; and an honest distinction between machine QC, first-listen
publication and full human listening.
