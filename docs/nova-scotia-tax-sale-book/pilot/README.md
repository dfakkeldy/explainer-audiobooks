# Narrated comprehension pilot

This folder contains a public-safe first-section candidate produced after Dan
approved the twelve-chapter outline and forty-figure direction for pilot
development.

Dan accepted the exact `first-section-candidate.md` teaching and voice with
“Let’s go for the voice.” at 2026-07-19T16:17:07-03:00. Its bytes are preserved
as `research/voice-exemplar.md`, SHA-256
`ee7c43ac59beafa462de4ec9d1135ccce78efa810dbbd2d5445274694213d5df`.

Dan approved the production-map-led thirteen-chapter/51-figure direction for
pilot development on 2026-07-20. `first-technical-passage.md` now adds the
authorized lien-to-notice teaching passage without becoming a canonical chapter
or full-book rough draft.

The governed Echo pilot rendered on 2026-07-20 at 866.645333 seconds (14:27).
Its ignored, nonpackage M4B is:

`.build/custom-learning-audiobooks/beyond-the-tax-sale-packet/pilot/dist/beyond-the-tax-sale-packet-pilot.m4b`

SHA-256:
`c94570d369b1c5f3842f111f151a9e4bb880db2d84ceeed86f3cfed44c974f1c`.
The alignment sidecar verified, the pronunciation audit is clean, and the
immutable renderer receipt still records listener acceptance as pending. The
next gate is Dan's separate `continue` or `revise` verdict against this exact
audio; no remaining manuscript drafting is authorized before that verdict.
