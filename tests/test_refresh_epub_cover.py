...
class RefreshEpubCoverTests(unittest.TestCase):
    ...
    def test_rebuilt_validation_rejects_changed_non_cover_metadata(self) -> None:
        with TemporaryDirectory() as raw_dir:
            root = Path(raw_dir)
            source_path = make_epub_fixture(root)
            cover_data = make_png(root / "new.png", (1600, 2560)).read_bytes()
            ...
            with zipfile.ZipFile(source_path) as source, zipfile.ZipFile(
                ...
                for source_info in source.infolist():
                    ...
                    payload = (
                        cover_data
                        if info.filename == "OEBPS/cover-old.png"
                        else source.read(source_info)
                    )
                    if info.filename == "OEBPS/chapter.xhtml":
                        info.comment = b"changed non-cover metadata"
                    rebuilt.writestr(info, payload)
            with zipfile.ZipFile(source_path) as source, zipfile.ZipFile(
                rebuilt_path
            ) as rebuilt:
                with self.assertRaisesRegex(ValueError, "non-cover metadata changed"):
                    refresh_epub_cover._validate_rebuilt_epub(
                        source,
                        rebuilt,
                        "OEBPS/cover-old.png",
