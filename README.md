...
**Model-agnostic, on purpose.** *Git Happens* was written by **DeepSeek v4** and the rest by **Opus 4.8**. The method doesn't care which model runs it; each book's EPUB metadata records the model that wrote it (as the `contributor`), with the human curator as the author.
...
Curated by **Dan Fakkeldy**. The worked example throughout is **[Echo](https://dfakkeldy.github.io/Echo/)**, an open-source audiobook study player. Books written by **Claude (Opus 4.8)** and **DeepSeek v4** via the `explainer-audiobook` skill.
